...
Q_GLOBAL_STATIC_WITH_ARGS(QByteArray,
    ...
    ("<style type=\"text/css\">\n"
     "\thtml { background-color: "
        ...
        + "; }\n"
          "\t.error { border-style: solid; border-width: 1px; border-color: #ff3b21; background-color: #ffe9c4; "
          "padding:7px;}\n"
          "\t.last  { border-style: solid; border-width: 1px; border-color: #2020ff; background-color: #e0e0ff; "
          "padding:7px;}\n"
...
bool ConsoleModel::addOperation(const Analitza::Expression &e, const QString &input) {
    ...
    if (a.isCorrect()) {
        ...
        addMessage(
            QStringLiteral("<a title='%1' href='kalgebra:/query?id=copy&func=%2'><span class='exp'>%3</span></a><br "
                           "/>= <a title='kalgebra:%1' "
        ...
        addMessage(i18n("<ul class='error'>Error: <b>%1</b><li>%2</li></ul>",
                       input.toHtmlEscaped(),
                       a.errors().join(u"</li>\n<li>"_s)),
            {},
            {});
...
bool ConsoleModel::loadScript(const QUrl &path) {
    ...
    if (correct)
        ...
        addMessage(i18n("<ul class='error'>Error: Could not load %1. <br /> %2</ul>",
                       path.toDisplayString(),
                       a.errors().join(u"<br/>"_s)),
            {},
            {});
...
void ConsoleModel::addMessage(const QString &msg,
    const Analitza::Expression &operation,
    const Analitza::Expression &result) {
